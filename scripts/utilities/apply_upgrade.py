...
Subcommands:
    ...
    prune         Template-only: move packs from the LIVE library to
                  docs/upgrades/archive/ once EVERY fleet project's ledger
                  records them (never deleted -- a pack is the record of a
                  defect and its fix, and archive/ is where an agent looks
                  for one, not git history; the template ledger gets a
                  Pruned row).
...
def cmd_prune(args: argparse.Namespace) -> int:
    ...
    if args.dry_run:
        print(f"[DRY-RUN] Would archive {len(absorbed)} pack(s) to docs/upgrades/archive/. "
              "No changes made.")
        return 0

    if not args.yes and not confirm(
        f"Archive {len(absorbed)} fully-absorbed pack(s) to docs/upgrades/archive/?"
    ):
        print("Aborting.")
        return 1
    ...
    for pack in absorbed:
        dest = archive_dir / pack.name
        try:
            # MOVE, never delete (owner decision 2026-08-30): a pack is a record of
            # a defect and its fix, and git history is not where an agent goes
            # looking for one -- docs/upgrades/archive/ is. A name collision means
            # the same pack was pruned before (impossible under normal operation,
            # since a pruned pack is gone from `absorbed`) or two differently-timed
            # packs collided on their timestamp suffix; refuse rather than silently
            # overwrite either one.
            if dest.exists() and not same_content(pack, dest):
                raise RuntimeError(
            ...
            row = build_ledger_row(
                ...
                "moved to docs/upgrades/archive/.",
            )
            append_ledger_row(ledger_path, read_ledger_text(directory), row)
            shutil.move(str(pack), str(dest))
            stage_paths(
                root,
                [f"docs/upgrades/{LEDGER_NAME}", f"docs/upgrades/{pack.name}",
                 f"docs/upgrades/archive/{pack.name}"],
            )
            print(f"[OK] Archived {pack.name} -> docs/upgrades/archive/{pack.name}")
        except (OSError, RuntimeError) as exc:
            failures += 1
            print(f"[ERROR] Failed to archive {pack.name}: {exc}")
