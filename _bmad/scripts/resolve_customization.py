...
def find_project_root(start: Path):
    # `_bmad/` ONLY, never `.git` (fix B, owner decision D11 2026-08-29, from the
    # skill_overlay_never_resolves pack). Registry-resident skills live inside a git
    # repo of their own, so a `.git` test made the walk stop AT THE REGISTRY, look for
    # `_bmad/custom/` there, find nothing, and silently return every skill's defaults
    # -- per-project overlays never resolved and nothing warned. The registry has no
    # `_bmad/`, so requiring it lets the skill_dir walk fail and the cwd fallback find
    # the INVOKING project, which is the one whose overlays were meant.
    #
    # NOTE: this file is installer-managed; the change diverges from upstream BMAD
    # until pushed there. Re-apply after any installer update.
