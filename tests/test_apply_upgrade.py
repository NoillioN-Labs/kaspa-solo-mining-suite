...
def _documented_record_commands() -> list[list[str]]:
    ...
    tick = chr(96)
    for pack in packs:
        # Fenced powershell blocks ONLY. Prose that quotes the command while explaining a past
        # defect is documentation, not an instruction, and must not be parsed as one.
        fenced, inside = [], False
        for line in pack.read_text(encoding="utf-8").splitlines():
            if not inside and line.strip().startswith(tick * 3 + "powershell"):
                inside = True
                continue
            if inside and line.strip().startswith(tick * 3):
                inside = False
                continue
            if inside:
                fenced.append(line)
